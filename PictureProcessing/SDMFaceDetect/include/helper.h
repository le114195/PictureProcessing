 ...
 */
static cv::Mat align_mean(cv::Mat mean, cv::Rect facebox, float scaling_x=1.0f, float scaling_y=1.0f, float translation_x=0.0f, float translation_y=0.0f)
{
    using cv::Mat;
 ...
 */
static cv::Rect perturb(cv::Rect facebox)
{
...
static std::string replace(const std::string& str, const std::string& dest, const std::string& src)
{
    std::string ret = str;
...
static void ReadLabelsFromFile(std::vector<ImageLabel> &Imagelabels, std::string Path = "labels_ibug_300W_train.xml"){
...
// 加载图像标签
static bool load_ImageLabels(std::string filename, std::vector<ImageLabel> &mImageLabels)
{
...
// 保存图像标签
static void save_ImageLabels(std::vector<ImageLabel> mImageLabels, std::string filename)
{
