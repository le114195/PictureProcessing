...
bool LinearRegressor::learn(cv::Mat &data, cv::Mat &labels, bool isPCA)
{
    ...
    if(this->isPCA){
        ...
        std::cout << FeaturePCA.eigenvectors.size() << std::endl;
        std::cout << FeaturePCA.eigenvalues.size() << std::endl;
        ...
        for(int i=0; i<FeaturePCA.eigenvalues.rows; i++){
            ...
            if(lamda/eigensum > 0.97){
                index = i;
                //std::cout << "Ãÿ’˜∏ˆ ˝ø…“‘—πÀıŒ™:" << i << "∏ˆ" << std::endl;
                break;
            }
        }
...
void ldmarkmodel::loadFaceDetModelFile(std::string filePath){
    ...
    if(face_cascade.empty()){
        std::cout << "face model empty!" << std::endl;
    }
}
...
void ldmarkmodel::train(std::vector<ImageLabel> &mImageLabels){
    ...
    std::cout << "«Î ‰»Î—µ¡∑—˘±æ∏ˆ ˝£¨◊Ó…Ÿ20∏ˆ." << std::endl;
    std::cin >> samplesNum;
    if(samplesNum < 20)
        samplesNum = 20;
    else if(samplesNum > mImageLabels.size())
        samplesNum = mImageLabels.size();
    std::cout << "“ªπ≤" << samplesNum << "∏ˆ—µ¡∑—˘±æ.\n" << std::endl;

    std::cout << " «∑Ò“‘¡Ω—€æ‡¿Î±Í◊ºπÈ“ªªØLandmarker◊¯±Í∆´≤Ó?\n[Y/N?]" << std::endl;
    std::string t;
    std::cin >> t;
    if(t=="Y" || t=="y")
        isNormal = true;
...
//            cv::waitKey(10);
    ...
    for(int i=0; i<samplesNum; i++){
        ...
        float distance = sqrt( (rx-lx)*(rx-lx)+(ry-ly)*(ry-ly) );//º∆À„¡Ω—€µƒæ‡¿Î
        for(int j=0; j<numLandmarks; j++){
...
int ldmarkmodel::track(const cv::Mat& src, cv::Mat& current_shape, bool isDetFace){
    ...
    if(src.channels() == 1){
        ...
        //cv::cvtColor(src, grayImage, CV_RGBA2GRAY);
        cv::cvtColor(src, grayImage, CV_BGRA2GRAY);
    }else{
        return SDM_ERROR_IMAGE;
    }
...
//    }
    if(isDetFace || faceBox.area()<100){
        std::vector<cv::Rect> mFaceRects;
        face_cascade.detectMultiScale(grayImage, mFaceRects, 1.1, 2, 0, cv::Size(100, 100));
        if(mFaceRects.size() <=0){
            current_shape = cv::Mat();
            return SDM_ERROR_FACENO;
        }
